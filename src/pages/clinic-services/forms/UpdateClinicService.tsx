...
const UpdateService = ({ open, setOpen, serviceId }: Props) => {
  const { trigger } = UpdateClinicServiceModel(serviceId);
  const { clinicService, isLoading } = useClinicService(serviceId);
  const { services } = useServices();
  const { clinics } = useClinics();

  const initialValues = {
    clinicId: clinicService?.data.clinicId,
    serviceId: clinicService?.data.serviceId,
    isAvailable: clinicService?.data.isAvailable,
    isSpecial: clinicService?.data.isSpecial,
    daysOffered: [
      {
        days: clinicService?.data.daysOffered?.[0].days,
        start: clinicService?.data.daysOffered?.[0].start,
        end: clinicService?.data.daysOffered?.[0].end,
      },
    ],
  };
  ...
  const onSubmit = async (values: any) => {
    ...
    await fetch("/api/update-clinic-service", {
